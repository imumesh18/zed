...
impl HttpTransport {
    ...
    pub fn with_config(base_url: Url, config: HttpTransportConfig, _cx: &AsyncApp) -> Result<Self> {
        ...
        let (_error_sender, error_receiver) = channel::bounded(config.channel_buffer_size);
        ...
        let transport = Self {
            ...
            mode: RwLock::new(TransportMode::Standard),
            message_sender,
            message_receiver,
            error_receiver,
        };
    ...
    async fn send_http_message(&self, message: String) -> Result<()> {
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow!("HTTP request failed: {} - {}", status, body));
        }
    ...
    async fn start_legacy_sse_mode(&self) -> Result<()> {
        ...
        let mode = Arc::new(RwLock::new(TransportMode::Standard));

        // Start the SSE connection in the background
        smol::spawn(async move {
            if let Err(e) = Self::handle_sse_connection(sse_url, client, message_sender, mode).await
            {
